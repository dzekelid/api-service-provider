---
name: Cloudify
x-slug: cloudify
description: Cloudify is an open source application and network orchestration framework
  based on TOSCA, that supports hybrid cloud deployments.
image: http://kinlane-productions.s3.amazonaws.com/screen-capture-api/11767-cloudify.jpg
x-kinRank: "7"
x-alexaRank: "556592"
tags: API Service Provider
created: "2018-08-25"
modified: "2018-08-25"
url: https://raw.githubusercontent.com/streamdata-gallery-topics/api-service-provider/master/_listings/cloudify/apis.md
specificationVersion: "0.14"
apis:
- name: Cloudify Manager API
  x-api-slug: cloudify-manager-api
  description: Cloudify is an open source application and network orchestration framework
    based on TOSCA, that supports hybrid cloud deployments.
  image: http://kinlane-productions.s3.amazonaws.com/screen-capture-api/11767-cloudify.jpg
  humanURL: http://getcloudify.org/
  baseURL: https:////http://localhost:8100
  tags: API Service Provider
  properties:
  - type: x-openapi-spec
    url: https://raw.githubusercontent.com/streamdata-gallery-topics/api-service-provider/master/_listings/cloudify/openapi.md
x-common:
- type: x-blog
  url: http://getcloudify.org/blog/
- type: x-blog-rss
  url: http://getcloudify.org/atom.xml
- type: x-crunchbase
  url: https://crunchbase.com/organization/cloudify-co
- type: x-documentation
  url: http://getcloudify.org/guide/3.0/rest-api/index.html
- type: x-email
  url: hello@getcloudify.org
- type: x-email
  url: hello@cloudify.co
- type: x-faq
  url: http://getcloudify.org/FAQ_cloud_devops_automation.html
- type: x-github
  url: https://github.com/CloudifySource
- type: x-privacy
  url: http://getcloudify.org/privacy_policy.html
- type: x-terms-of-service
  url: http://getcloudify.org/terms_of_use.html
- type: x-twitter
  url: https://twitter.com/CloudifySource
- type: x-website
  url: http://getcloudify.org/
include: []
maintainers:
- FN: Kin Lane
  x-twitter: apievangelist
  email: info@apievangelist.com
---